...
function initSupabase(){
  if (window.supabase?.createClient && SUPABASE_URL.includes('https://') && !SUPABASE_URL.includes('YOUR_SUPABASE')) {
    state.client = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
  }
}

async function login(){
  if (!state.client) return showToast('Configure Supabase dans app.js');
