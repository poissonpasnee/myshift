...
function renderCalendar(){
  ...
  var grid=gid('calendarGrid'); grid.innerHTML=''; grid.classList.remove('slide-in'); void grid.offsetWidth; grid.classList.add('slide-in');
...
function extractSchedule(rows){
  ...
  var dates=hdr.map(function(c){var m=String(c).match(/(\\d{1,2})\\/-(?:\\/-)?/);if(!m)return null;var y=m[3]?(m[3].length===2?2000+ +m[3]: +m[3]):currentYear;return keyOf(y,+m[2]-1,+m[1]);});
  var res=[];
